...
public:
  ...
  tlm::tlm_fifo<mgc_sysc_ver_array1D<int,66880> >                TLS_in_fifo_image_in;
  tlm::tlm_fifo<mc_wait_ctrl>                                    TLS_in_wait_ctrl_fifo_image_in;
  mc_trios_input_monitor                                         trios_monitor_image_in_rsc_triosy_lz_INST;
  mc_input_transactor<mgc_sysc_ver_array1D<int,66880>,32,true>   transactor_image_in;
  tlm::tlm_fifo<mgc_sysc_ver_array1D<int,66880> >                TLS_out_fifo_image_out;
  tlm::tlm_fifo<mc_wait_ctrl>                                    TLS_out_wait_ctrl_fifo_image_out;
  mc_trios_output_monitor                                        trios_monitor_image_out_rsc_triosy_lz_INST;
  mc_output_transactor<mgc_sysc_ver_array1D<int,66880>,32,true>  transactor_image_out;
  ...
  scverify_top(const sc_module_name& name)
    ...
    , image_out_rsc_INST("image_out_rsc", true)
    ...
    , transactor_image_in("transactor_image_in", 0, 32, 0, false)
    , TLS_out_fifo_image_out("TLS_out_fifo_image_out", -1)
    , TLS_out_wait_ctrl_fifo_image_out("TLS_out_wait_ctrl_fifo_image_out", -1)
    , trios_monitor_image_out_rsc_triosy_lz_INST("trios_monitor_image_out_rsc_triosy_lz_INST")
    , transactor_image_out("transactor_image_out", 0, 32, 0)
    ...
    , sync_generator_INST("sync_generator", true, false, false, false, 200851, 200851, 0)
    , catapult_monitor_INST("Monitor", clk, true, 0LL, 200851LL)
    ...
    , deadlocked("deadlocked")
  {
    ...
    transactor_image_in.bind_clk(clk, true);
    ...
    trios_monitor_image_out_rsc_triosy_lz_INST.trios(TLS_image_out_rsc_triosy_lz);
    trios_monitor_image_out_rsc_triosy_lz_INST.register_mon(&catapult_monitor_INST);

    transactor_image_out.out_fifo(TLS_out_fifo_image_out);
    transactor_image_out.out_wait_ctrl_fifo(TLS_out_wait_ctrl_fifo_image_out);
    transactor_image_out.bind_clk(clk, true);
    transactor_image_out.add_attribute(*(new sc_attribute<int>("MC_TRANSACTOR_EVENT", 0 )));
    transactor_image_out.register_block(&image_out_rsc_INST, image_out_rsc_INST.basename(), TLS_image_out_rsc_triosy_lz,
        0, 66879, 1);

    testbench_INST.clk(clk);
    testbench_INST.ccs_image_in(TLS_in_fifo_image_in);
    testbench_INST.ccs_wait_ctrl_image_in(TLS_in_wait_ctrl_fifo_image_in);
    testbench_INST.ccs_image_out(TLS_out_fifo_image_out);
...
void scverify_top::start_of_simulation() {
  ...
  if (SCVerify_AUTOWAIT) {
    ...
    transactor_image_out.set_auto_wait_limit(l);
  }
}
...
void scverify_top::setup_debug() {
  ...
  static int transactor_image_out_flags = MC_DEFAULT_TRANSACTOR_LOG;
#else
  static int transactor_image_in_flags = MC_TRANSACTOR_UNDERFLOW | MC_TRANSACTOR_WAIT;
  static int transactor_image_out_flags = MC_TRANSACTOR_UNDERFLOW | MC_TRANSACTOR_WAIT;
#endif
  static int transactor_image_in_count = -1;
  static int transactor_image_out_count = -1;
  ...
  if (debug_cmds.is_open()) {
    ...
    debug("transactor_image_out",transactor_image_out_flags,transactor_image_out_count);
  }
}
...
void scverify_top::debug(const char* varname, int flags, int count) {
  ...
  if (strcmp(varname,"transactor_image_out") == 0)
    xlator_p = &transactor_image_out;
  if (xlator_p) {
...
void scverify_top::generate_reset() {
  ...
  if (first || sc_time_stamp() == SC_ZERO_TIME) {
    ...
    transactor_image_out.reset_streams();

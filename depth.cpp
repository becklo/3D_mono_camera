...
void creation_error(int image_floue[IMG_WIDTH][IMG_HEIGHT], int image_net[IMG_WIDTH][IMG_HEIGHT],
  ...
  std::cout << "conv done" << '\n';
  ecriture_image_conv("conv.pgm",result_conv);
  //sleep(20);
...
void reconstruction_error(int rec_error_tab[IMG_WIDTH][IMG_HEIGHT],int result_rec_error[IMG_WIDTH][IMG_HEIGHT],int i, int j){
  int rec_error = 0;
  for (int l = i - SIZE_WI; l < i + SIZE_WI; l++){
    ...
    result_rec_error[j][i]=rec_error;
